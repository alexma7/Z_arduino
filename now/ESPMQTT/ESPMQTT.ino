...
const char* password = "noparol93";
//const char* ssid = "UPVEL";
//const char* password = "12345678";

//MQTT
//const char *mqtt_server = "192.168.1.39"; // Имя сервера MQTT
...
void sendToMQTT ()
{
  ...
  if (strAdr != "" || strVal != "" )
  {
	  //if (strAdr=="/hall/light1")
	  //{
	//	client.unsubscribe("strAdr");
	//	Serial.print("zashel");
	 // }
	  //else
	  //{
...
void callback(char* topic, byte * payload, unsigned int length) {
  ...
  strCallback+="%";
 /* if (strCallback!=TopicCheck)
  {
	strCallback=TopicCheck;
	Serial.println(strCallback);	
  }*/
  //Serial.println('&');
  Serial.println(strCallback);

  // Вкл выкл светодиод на ESP
 /* if ((char)payload[0] == '1') {
    rState1 = true;
...
void TempSend() {
  if (tm == 0)
  {
    ...
    char test[20];
    client.publish("/home/hall/temper", dtostrf(dht.readTemperature(), 2, 2, test));  // отправляем в топик для термодатчика значение температуры
    client.publish("/home/hall/humid", dtostrf(dht.readHumidity(), 2, 2, test));
    //Serial.println(temp);
